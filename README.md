...
![Infrastructure Diagram](./F5_XC_CE_Infrastructure.svg)

**[📝 Edit Diagram](./F5_XC_CE_Infrastructure.drawio)** - Open the `.drawio` file to modify the diagram

**Diagram Components**:
- 🔵 **Azure Resources**: VNETs, subnets, load balancers, and virtual machines with official Azure Architecture Icons V22
...
- 🎨 **Visual Standards**: Microsoft Learn-compliant styling with proper Azure brand colors
...
**Automatic Updates**: Both the SVG diagram and `.drawio` source file are automatically regenerated whenever infrastructure changes are applied via Terraform, ensuring they always reflect the current deployed state.

> 💡 **Note**: Diagram files use SVG format for scalable, high-quality rendering with embedded Azure Architecture Icons V22. Both files are version-controlled in Git, providing a visual history of infrastructure changes over time.
